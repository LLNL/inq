...
real_time::result propagate(systems::ions & ions, systems::electrons & electrons, const input::interaction & inter, const input::rt & options, IonSubPropagator const& ion_propagator = {}){
		...
		energy.eigenvalues = operations::sum(electrons.occupations_, eigenvalues, [](auto occ, auto ev){ return occ*real(ev); });
		...
		if(electrons.phi_.fields().full_comm().root()) tfm::format(std::cout, "step %9d :  t =  %9.3f  e = %.12f\n", 0, 0.0, energy.total());
		...
		for(int istep = 0; istep < numsteps; istep++){
			...
			
			{
				...
				electrons.density_ = density::calculate(electrons.occupations_, fullstep_phi, electrons.density_basis_);
				ham.scalar_potential = sc.ks_potential(electrons.density_, energy);
			}
			...
			if(electrons.phi_.fields().full_comm().root()) tfm::format(std::cout, "step %9d :  t =  %9.3f  e = %.12f  wtime = %9.3f\n", istep + 1, (istep + 1)*dt, energy.total(), elapsed_seconds.count());
