...
real_time::result propagate(systems::ions & ions, systems::electrons & electrons, const input::interaction & inter, const input::rt & options, IonSubPropagator const& ion_propagator = {}){
		...
		auto occupations = electrons.states_.occupations()({electrons.phi_.set_part().start(), electrons.phi_.set_part().end()});
		
		electrons.density_ = density::calculate(occupations, electrons.phi_, electrons.density_basis_);
		...
		energy.eigenvalues = operations::sum(occupations, eigenvalues, [](auto occ, auto ev){ return occ*real(ev); });
		...
		for(int istep = 0; istep < numsteps; istep++){
			...
			
			{
				...
				electrons.density_ = density::calculate(occupations, fullstep_phi, electrons.density_basis_);
				ham.scalar_potential = sc.ks_potential(electrons.density_, energy);
			}
