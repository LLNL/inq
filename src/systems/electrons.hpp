...
public:
	...
	electrons(input::parallelization const & dist, const inq::systems::ions & ions, const input::config & conf = {}, KptsType const & kpts = input::kpoints::gamma()):
		...
		atomic_pot_(ions.num_atoms(), ions.atoms(), states_basis_.gcutoff(), conf.double_grid_value()),
	...
	void print(const inq::systems::ions & ions){
		...
		if(logger()){
			...
			if(ions.num_atoms() > 0) logger()->info("system symmetries: " + ions.symmetry_string());
