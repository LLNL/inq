...
namespace hamiltonian {
  ...
  public:
    ...
    void operator()(const field_set_type & phi, field_set_type & vnlphi) const {
      ...
      boost::multi::array<typename field_set_type::value_type, 2> sphere_phi({sphere_.size(), phi.set_size()});

      sphere_.gather(phi.cubic(), sphere_phi);

      boost::multi::array<typename field_set_type::value_type, 2> projections({nproj_, phi.set_size()});
