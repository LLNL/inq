...
int main(int argc, char ** argv){
	...
	//Fourier space pseudo
	{
		...
		auto energy = ground_state::calculate(electrons, input::interaction::non_interacting() | input::interaction::fourier_pseudo(), input::scf::conjugate_gradient());
      ...
      Non-local   =       -18.00231356
		...
		*/
		...
		energy_match.check("total energy",     energy.total()    , -66.260998365817);
		energy_match.check("kinetic energy",   energy.kinetic()  ,  35.546492603119);
		energy_match.check("eigenvalues",      energy.eigenvalues, -61.740957352732);
		energy_match.check("external energy",  energy.external   , -79.409319159042);
		energy_match.check("non-local energy", energy.nonlocal   , -17.878130796809);
		energy_match.check("ion-ion energy",   energy.ion        ,  -4.520041013085);
