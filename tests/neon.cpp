	...
	//Fourier space pseudo
	{
      ...
      Non-local   =       -18.00231356
		...
		*/
		...
		energy_match.check("total energy",     result.energy.total()    , -66.260998365817);
		energy_match.check("kinetic energy",   result.energy.kinetic()  ,  35.546492603119);
		energy_match.check("eigenvalues",      result.energy.eigenvalues, -61.740957352732);
		energy_match.check("external energy",  result.energy.external   , -79.409319159042);
		energy_match.check("non-local energy", result.energy.nonlocal   , -17.878130796809);
